...
### MODIFICATION
    def tolist(self) -> List:
        """        
        returns a list representation of half of the board board using the specified values for pieces

        (intended to be used as features for NN)

        result is a 32 x 1 list
        """
        board = []
...
### MODIFICATION:
    ...
    def generate_pseudo_legal_moves_lanuci(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[str]:
            ...
            # if self.ep_square:
            #     yield from self.generate_pseudo_legal_ep(from_mask, to_mask)
    def generate_pseudo_legal_moves_pan(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[str]:
            our_pieces = self.occupied_co[self.turn]
...
### MODIFICATION: copied iterator generation for string representations
    def generate_legal_moves_uci(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[str]:
            ...
            if king_mask:
                ...
                yield from self.generate_pseudo_legal_moves_uci(from_mask, to_mask)

    def generate_legal_moves_lanuci(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[str]:
            if self.is_variant_end():
                return
            ...
            if king_mask:
                ...
                if checkers:
                    for move in self._generate_evasions(king, checkers, from_mask, to_mask):
                        if self._is_safe(king, blockers, move):
                            yield str(self.piece_at(move.from_square)).upper() + str(move)
                else:
                    for move in self.generate_pseudo_legal_moves(from_mask, to_mask):
                        if self._is_safe(king, blockers, move):
                            yield str(self.piece_at(move.from_square)).upper() + str(move)
            else:
                yield from self.generate_pseudo_legal_moves_lanuci(from_mask, to_mask)

    def generate_legal_moves_pan(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[str]:
            if self.is_variant_end():
                return
            ...
            if king_mask:
                ...
                if checkers:
                    for move in self._generate_evasions(king, checkers, from_mask, to_mask):
                        if self._is_safe(king, blockers, move):
                            yield str(self.piece_at(move.from_square)).upper() + SQUARE_NAMES[move.to_square]
                else:
                    for move in self.generate_pseudo_legal_moves(from_mask, to_mask):
                        if self._is_safe(king, blockers, move):
                            yield str(self.piece_at(move.from_square)).upper() + SQUARE_NAMES[move.to_square]
            else:
                yield from self.generate_pseudo_legal_moves_pan(from_mask, to_mask)
###
...
### MOFICIATION
    def to_uci(self) -> Iterator[str]:
        return self.board.generate_legal_moves_uci()

    def to_lanuci(self) -> Iterator[str]:
        return self.board.generate_legal_moves_lanuci()

    def to_pan(self) -> Iterator[str]:
        return self.board.generate_legal_moves_pan()
###
